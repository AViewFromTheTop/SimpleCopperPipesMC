...
public class CopperPipeListener extends VibrationListener {
    public static Codec<CopperPipeListener> createPipeCodec(CopperPipeListener.VibrationListenerConfig callback) {
        return RecordCodecBuilder.create((instance) -> instance.group(
                ...
                .apply(instance, (positionSource, integer, optional, float_, integer2) -> new CopperPipeListener(positionSource, integer, callback, optional.orElse(null), float_, integer2)));
    }
            ...
            if (!this.config.isValidVibration(gameEvent, context)) {
                ...
                if (optional.isEmpty()) {
                    ...
                    if (!this.config.shouldListen(serverLevel, this, new BlockPos(vec3), gameEvent, context)) {
                        ...
                        this.scheduleSignal(gameEvent, context, vec3, vec32);
                        return true;
                    }
                }
            }
    ...
    private void scheduleSignal(GameEvent gameEvent, GameEvent.Context context, Vec3 vec3, Vec3 vec32) {
